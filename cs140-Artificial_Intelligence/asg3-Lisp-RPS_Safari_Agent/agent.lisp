;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; $Id: agent.lisp,v 1.1 2012-02-13 00:44:44-08 dmfrank - $
...
(let ((hist-info
  ...
  ;;;;
  ;; AGENT - function
  (defun dmfrank-p3 (history scores myscore)
  ;;;;;;
  ;;; HELPER FUNCTIONS
    ;;;;
    ...
    (defun feedback (play totals)
      (if (or (null play) (null totals)) 0
        (let ((sign (pop play)) (pick (pop play))
          ...
          (if (>= sign 1)
            (cond
              ((eql pick 'R) (cond ((> totS totP) 1) ((> totP totS) 0) (t 1/4)))
              ((eql pick 'P)(cond ((> totR totS) 1) ((> totS totR) 0) (t 1/4)))
              (t (cond ((> totP totR) 1) ((> totR totP) 0) (t 1/2))))
            (cond
              ((eql pick 'R) (cond ((< totS totP) 1) ((< totP totS) 0) (t 1/4)))
              ((eql pick 'P) (cond ((< totR totS) 1) ((< totS totR) 0) (t 1/4)))
              (t (cond ((< totP totR) 1) ((< totR totP) 0) (t 1/4))))))))
    ;;;;
    ;; WHO-WON - strategic
    ...
    (defun attain-strat (sub-perf totals)
      (let ((sub1 (pop sub-perf)) (sub2 (pop sub-perf)) (sub3 (pop sub-perf))
        ...
        (if (not (null totals))
          (progn
            (setf sub1 (update-perf sub1 totals 16))
            (setf sub2 (update-perf sub2 totals 16))
            (setf sub3 (update-perf sub3 totals 16))
            (setf sub4 (update-perf sub4 totals 16))))
        (setf perf1 (car sub1)) (setf perf2 (car sub2))
        (setf perf3 (car sub3)) (setf perf4 (car sub4))
        (cond
          ((>= perf4 perf2 perf3 perf1 3/20) (setf gn 4))
          ((>= perf3 perf2 perf1 perf4 3/20) (setf gn 3))
          ((>= perf2 perf1 perf3 perf4 3/20) (setf gn 2))
          ((>= perf1 perf2 perf3 perf4 3/20) (setf gn 1))
          (t (setf gn (nth (random 3) '(4 3 2)))))
    ...
    (defun historical-agent (sub-perf mem hist gn)
      ...
      (let ((sub1 (pop sub-perf)) perf1 play1 gplay1
        ...
        (cond
          ((>= perf8 perf4 perf2 perf1 3/20) (setf base play8) (setf pick gplay8))
          ((>= perf4 perf8 perf2 perf1 3/20) (setf base play4) (setf pick gplay4))
          ((>= perf2 perf4 perf8 perf1 3/20) (setf base play2) (setf pick gplay2))
          ((>= perf1 perf4 perf2 perf8 3/20) (setf base play1) (setf pick gplay1))
    ...
    ;;;;
    ;; MAIN
    (let (sign bet pick base (recent (car history)) winners (lastplay '(1 R))
      ...
      (cond
        ...
        ((< (length hist-mem) 1024) (if (not (null history)) (push recent hist-mem)))
        ((and (< (car hist-perf) 1/3) (> (length hist-mem)) 512)
          (setf hist-mem (reverse hist-mem)) (dotimes (i 32) (pop hist-mem))
      ...
      (if (and (> (car hist-perf) 2/5) (= (feedback (cadr hist-perf) recent) 1))
            ;(= (feedback (cadr hist-perf) recent) 1))
        (cond
